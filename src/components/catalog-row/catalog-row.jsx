...
const CatalogPostMedia = ({post}) => {
  ...
  if (mediaType === 'image') {
    return <div style={style} className={styles.mediaWrapper}><img style={style} className={styles.media} src={post?.link} alt='' /></div>
  }
  if (mediaType === 'video') {
    return <div style={style} className={styles.mediaWrapper}><video style={style} className={styles.media} controls={true} autoPlay={false} src={post?.link} /></div>
  }
  return <div className={styles.noMedia}></div>
}
