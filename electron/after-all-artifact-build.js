...
const addPortableToPortableExecutableFileName = () => {
  ...
  for (const file of files) {
    if (file.endsWith('.exe') && !file.match('Setup')) {
      ...
      const renamedFilePath = path.resolve(distFolderPath, file.replace('plebones', 'plebones Portable'))
...
const createHtmlArchive = () => {
  ...
  const plebonesHtmlFolderName = `plebones-html-${packageJson.version}`
  const outputFile = path.resolve(distFolderPath, `${plebonesHtmlFolderName}.zip`)
  ...
  try {
    ...
    // rename 'build' folder to 'plebones-html-version' inside the archive
    execSync(`${zipBinPath} rn -r ${outputFile} build ${plebonesHtmlFolderName}`)
  } catch (e) {
